...
const getEntityMetadata = (
  ...
  const labels = (get(issue, ['labels'], []) || []).map((label) =>
    pick(label, ['id', 'name'])
  )

  const assignees = (get(issue, ['assignees'], []) || []).map((member) =>
    pick(member, ['username', 'name'])
  )

  const project = (projectOptions || []).find(
    ({ value }) => Number(value) === Number(issue.project_id)
  )
