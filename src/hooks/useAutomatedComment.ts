...
const useAutomatedComment: UseAutomatedComment = () => {
  ...
  const dontAddComment =
    get(context, ['settings', 'dont_add_comment_when_linking_issue']) === true

  const createAutomatedLinkedComment = useCallback(
    (projectId: Project['id'], issueIid: Issue['iid']) => {
      if (!client || dontAddComment || !ticketId || !permalink) {
